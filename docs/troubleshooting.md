...
| `Sig` column shows `~` | Too few samples for the [Mann-Whitney U test](https://en.wikipedia.org/wiki/Mann%E2%80%93Whitney_U_test) (requires ≥5 per group) | Increase iterations or combine more runs - see [FAQ: significance](./faq.md#why-is-significance-sometimes-blank) |
...
| "Could not instantiate MyClass" | No public parameterless constructor | Add one, use `[BenchmarkSetup]`, or add `NBenchmark.Extensions.DependencyInjection` - see [FAQ: instantiation](./faq.md#the-host-throws-could-not-instantiate-myclass-how-do-i-fix-it) |
